...
//! reworded, but a `case $?` somebody wrote against basis a year ago has to keep
...
use basis_core::RunReport;

/// The run finished.
pub(crate) const EXIT_OK: u8 = 0;
/// The run failed, or basis could not start it.
...
mod tests {
    ...
    use basis_core::RunOutcome;

    /// A report with nothing in it but the two fields the exit code reads.
    fn report(outcome: RunOutcome, stopped_by: Option<basis_core::Bound>) -> RunReport<()> {
        RunReport {
            ...
            outcome,
            stopped_by,
            usage: basis_core::RunUsage::default(),
            sink: (),
        }
    }
    ...
    fn a_tripped_bound_is_told_apart_from_a_failure_by_the_exit_code() {
        // The whole point of the contract: `basis spawn --deadline 10m …; case $? in`
        ...
        let bounded = report(
            ...
            Some(basis_core::Bound::Deadline),
        );
    ...
    fn a_run_that_answered_on_a_spent_token_budget_still_exits_bounded() {
        ...
        let answered = report(RunOutcome::Ok, Some(basis_core::Bound::TokenBudget));
        let unanswered = report(
            RunOutcome::Error {
                message: "run completed without a final assistant message".to_string(),
            },
            Some(basis_core::Bound::TokenBudget),
        );
