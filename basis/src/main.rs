//! The `basis` binary: a thin shell over [`basis_core`] and [`basis_acp`].
//!
...
//! basis spawn "<prompt>"      -> enqueue one task and print its durable handle
//! basis send <ID> "<message>" -> enqueue a later turn for a running task
//! basis ask <ID> "<question>"  -> enqueue a turn and await its reply
//! basis wait <ID>              -> observe a repeatable terminal result
//! basis wait <ID> --message <MID>
...
//! escapes a prompt that collides with a subcommand name: `basis -- run`.
//!
//! # Where the parts are
//!
