...
//!
//! `basis spawn` has three possible fates, and until ADR-0020 the choice between
//! them fell out of `--json` — a *rendering* flag deciding whether work ran at
//! all. That put the human on the worst cell of the grid: `basis "hi"` minted a
//! checkpoint nothing was driving and printed a handle, while `basis --json "hi"`
...
/// Reads the matrix above. `in_lan_task` is whether `BASIS_TASK_ID` named a task
