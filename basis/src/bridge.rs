...
//! pipe and nothing more — a socket, a handshake, and the transport swap. basis
...
//! granted it runs commands as whoever started `basis`. There is no
...
//! loopback socket is already running as this user and could simply run `basis`,
//! so a bound would cost a real client an unexplained refusal to deny an
//! attacker nothing.
//!
//! # Why this lives in the binary
//!
//! It knows nothing about basis. Every ACP agent needs the same relay to reach a
//! browser, and this one would work over any of them: it depends on the
//! protocol's transport traits and on `basis-acp`'s [`serve`](basis_acp::serve),
...
//! somewhere to go, and it is never an identity argument for basis (ADR-0011).
