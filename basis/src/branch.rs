...
//! already paid for.
//!
//! # basis owns the shape
//!
//! [`TranscriptEntry`] is basis's type rather than a re-export of mentra's
//! `TranscriptItem`, for the same reason [`Event`](crate::Event) and
//! [`TurnOptions`](crate::TurnOptions) are basis's: one normalized surface every
...
//! is the stricter check of the two: an id basis cannot find is an id basis will
//! not act on.
//!
...
/// What an entry is. basis's own naming of mentra's transcript kinds.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
...
mod tests {
    ...
    fn an_archived_entry_is_still_a_branch_point() {
        ...
        // Addressable *and* returnable-to. basis used to refuse this itself,
