...
//! is rendered as.
//!
//! The renderer lives here rather than next to [`basis_core`]'s events because it
...
pub(crate) async fn execute_run(args: RunArgs) -> Result<ExitCode, String> {
    ...
    if args.json {
        let report =
            basis_core::run_with_approver(config, JsonlWriter::new(io::stdout()), approver)
                .await
    ...
    let report = basis_core::run_with_approver(config, prose_sink(), approver)
        .await
        .map_err(|error| error.to_string())?;
