...
//! longer exists.
//!
//! One test crate rather than four, which is what a directory with a `main.rs`
//! buys: [`store_outage`] is the shared fixtures put through a store that goes
//! read-only mid-run, [`scoped_rules`] is a durable remembered rule against a
//! session authorizer that refuses it, and [`side_effect_classification`] is
//! the harness asked about a declared tool that leaves the machine — and a
//! second, third and fourth `tests/*.rs` would each compile and link their own
//! copy of the scripted-provider harness below.

mod scoped_rules;
mod side_effect_classification;
#[cfg(unix)]
mod store_outage;
...
use basis::{
    ...
    Event, ToolSideEffectLevel, approval::ApprovalGate, run::prepare_with_session,
};
use mentra::{
