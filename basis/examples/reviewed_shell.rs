...
//! [`Approver`](basis::Approver). The obvious next question is who answers
...
//! [`OutputSpec`](basis::OutputSpec) — no `with_tools()` — and a typed turn
...
//! cargo run -p basis --example reviewed_shell -- /tmp/scratch
//! ```
//!
...
/// by `basis`. What makes the rung reachable at all is that
