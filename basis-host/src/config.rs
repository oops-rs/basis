//! Where a served host's sessions come from, and what its client cannot say.
//!
//! [`SessionSource`] is the concrete host seam served adapters consume;
//! [`SessionTemplate`] and [`Discovery`] are the operator-owned inputs used by
//! basis's configured source.
//!
...
//! a protocol server gets its own copy.
...
/// Where a hosted session's [`PreparedRun`] comes from.
///
/// The same seam as [`prepare_with_session`](basis::run::prepare_with_session),
/// at the host layer: a Rust process that already owns a mentra runtime —
...
pub struct SessionTemplate {
    pub(crate) provider: Option<BuiltinProvider>,
    pub(crate) base_url: Option<String>,
    pub(crate) model: Option<ModelSelector>,
