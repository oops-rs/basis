//! `lan watch`, deleted as a feature and rebuilt as a page of host code.
//!
...
//! first was ever a scheduler opinion, and it was never lan's to hold — so the
...
//! If the loop below ever stops being trivial, the regression is in lan's API
...
//! cargo run -p lan-core --example watch -- /repo "fix whatever the tests say" 60
//! ```

use std::{env, time::Duration};

use lan_core::{ModelSelector, NullSink, RunOutcome, RunReport, RunSpec, Snapshot, Workspace};

/// What one iteration may spend.
///
/// Stated here because nothing states it for us: with no scheduler shipped
/// there is no period for lan to guess a deadline from, so ADR-0014 makes
