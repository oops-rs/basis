...
//!
//! `cargo run -p lan-core --example conversation` — needs LAN_API_KEY and usually
//! LAN_BASE_URL/LAN_MODEL. Proves against a real provider what
//! `tests/conversation.rs` proves against a mock: the second turn sees the
//! first, because the session survives.

use lan_core::{AllowAll, CollectingSink, NullSink, RunConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(base_url) = std::env::var("LAN_BASE_URL") {
