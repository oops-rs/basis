...
impl Scenario {
    ...
    /// which is exactly how [`lan_core::WorkspaceBuilder::open`] installs it.
...
/// that has never heard of `.lan/hooks.json` ends up with.
...
// where both are present and lan's ordering has to hold across them. Gated to
...
async fn an_interceptors_rewrite_is_still_re_checked_by_a_hook() {
    // The no-smuggling property `PreExecutionHooks::run` documents. lan's
    // runner owns the ordering, so it is lan's runner that has to keep the
