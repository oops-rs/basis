#![cfg(unix)]
//! Subprocess hooks, end to end: a real `.lan/hooks.json`, real scripts on
//! disk, real processes.
//!
...
//! above them proves lan's half of the contract and none of it proves that the
...
fn a_hook_is_told_which_schema_it_is_talking_to() {
    ...
    let script = workspace.script(
        ...
        case "$request" in
            ...
            *) echo '{"decision":"deny","reason":"unknown lan"}' ;;
        esac
        "#,
    );
    workspace.hooks_file(&format!(
...
//
// Everything above proves lan answers correctly when asked. These two prove the
