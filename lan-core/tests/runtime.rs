...
//!    pre-hook is the agent's `base_dir` — the assumption lan's per-workspace
...
/// today a shared runtime tags every row `"lan:runtime"`, so per-workspace
...
mod roster {
    ...
    async fn a_foreign_mcp_tool_never_reaches_this_workspaces_roster() {
        ...
        assert!(
            ...
            "the roster parsed: lan's own tool must be in it: {offered:?}"
        );
        assert!(
...
mod dispatch_key {
    ...
    /// Pins the assumption lan's hook dispatcher is built on: what mentra
    /// hands a pre-hook as `working_directory` is the agent's `base_dir` —
    /// the same path lan keys its workspace registry with. If this ever moves
...
async fn a_workspaces_hooks_guard_its_runs_on_a_shared_runtime() {
    ...
    std::fs::create_dir_all(guarded.path().join(".lan")).expect("dir");
    std::fs::write(
        guarded.path().join(".lan/hooks.json"),
        format!(
