...
//! run, stopped by mentra at a real round boundary, arrives back in lan
...
async fn a_run_stopped_by_its_token_budget_names_the_budget() {
    ...
    // says otherwise.
    let lan_core::RunOutcome::Error { message } = &report.outcome else {
...
async fn a_shared_allowance_drawn_dry_stops_the_run_the_same_way() {
    ...
    // is computed by lan (`turn_bound`) rather than passed through, and a pool
    ...
    let pool = lan_core::BudgetPool::new(1);
