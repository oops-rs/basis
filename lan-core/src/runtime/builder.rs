...
const SHARED_IDENTIFIER: &str = "lan:runtime";
...
impl RuntimeBuilder {
    ...
    /// exchanged should not have to export an environment variable for lan to
    ...
    /// current directory** rather than by any workspace lan opened — so a host
    ...
    /// which lan sets per workspace. What this policy grants is command
    ...
    fn build_with(self, identifier: String, policy: RuntimePolicy) -> Result<Runtime, RunError> {
        ...
        let builder = mentra::Runtime::builder()
            ...
            // mentra tags every agent `"default"` and lan's own listing —
            ...
            // The one pre-hook lan registers, always: mentra takes hooks at
...
/// lan does not ship one. On shared runtimes the same rule is enforced by the
