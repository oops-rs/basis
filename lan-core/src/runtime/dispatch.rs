...
//! shared runtime is built before any workspace opens. So lan registers exactly
...
//! agent's `base_dir` — the same path lan scoped the agent to at open. Both
//! sides canonicalize through one helper, so a symlinked spelling and its
//! target land on one entry.
//!
//! # What runs, in what order
//!
//! On a hit: lan's own guards first — the shell posture and the `.git`
...
//! lan adds here were policy-level hygiene rather than a boundary to begin
...
/// The one [`PreExecutionHook`] a lan runtime registers.
...
/// mentra owns and the registry lan keeps a handle on.
...
/// lan's own guards, ahead of every configured participant.
///
