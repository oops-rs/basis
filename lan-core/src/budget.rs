...
/// use lan_core::{BudgetPool, CollectingSink, Workspace};
///
...
pub struct BudgetPool {
    ...
    /// `SeqCst` throughout, matching mentra's own accesses, so lan's reads sit
...
mod tests {
    ...
    fn a_pool_is_exhausted_the_moment_its_limit_is_reached() {
        // mentra's comparison is `reported >= budget`, and lan's has to be the
