...
fn config(workspace: &Path, prompt: &str) -> RunConfig {
    RunConfig::new(workspace, prompt).with_context(basis_core::ContextConfig {
...
async fn an_entry_this_conversation_does_not_have_is_refused() {
    ...
    let error = prepared
        ...
        .expect_err("an id basis cannot find is an id basis will not act on");
    ...
    assert!(
        ...
        "asking about an id basis does not have is not an error, just an empty answer"
    );
}
...
async fn an_abandoned_entry_is_offered_and_accepted() {
    ...
    // `branch_from` accepts. Until mentra 0.16 it was not: basis showed the
...
async fn a_branch_announces_itself_on_the_session_stream() {
    ...
    // branch subscribes itself. The event it gets is basis's own, mapped by the
...
async fn a_conversation_can_return_to_an_abandoned_branch() {
    ...
    // Now go back. This is what basis refused to attempt before mentra 0.16:
