...
/// A runtime built the way [`basis_core::WorkspaceBuilder::open`] builds one:
/// `spawn` registered, the approval gate installed, and commands allowed or
/// not exactly as [`basis_core::ShellAccess`] would have set them.
...
fn config(workspace: &Path) -> RunConfig {
    RunConfig::new(workspace, "do the thing").with_context(basis_core::ContextConfig {
