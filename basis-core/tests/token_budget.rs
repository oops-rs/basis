...
fn config(workspace: &Path) -> RunConfig {
    RunConfig::new(workspace, "make a file").with_context(basis_core::ContextConfig {
...
async fn a_run_stopped_by_its_token_budget_names_the_budget() {
    ...
    // says otherwise.
    let basis_core::RunOutcome::Error { message } = &report.outcome else {
...
async fn a_shared_allowance_drawn_dry_stops_the_run_the_same_way() {
    ...
    let pool = basis_core::BudgetPool::new(1);
