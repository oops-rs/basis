...
//!
//! 1. **Spending reaches the pool as it happens.** The pool is not a tally basis
...
//! does with `token_budget: Some(0)` — the alternative basis rejected. That
...
async fn a_pooled_run_and_its_own_report_agree_on_what_it_spent() {
    ...
    // through basis's event stream. Two paths, and they have to arrive at the
...
async fn a_second_prompt_on_one_conversation_draws_on_the_same_pool() {
    ...
    let taken = run
        .send("second", CollectingSink::default(), basis_core::AllowAll)
        .await;
    assert!(
        taken.is_ok(),
    ...
    let third = run
        .send("third", CollectingSink::default(), basis_core::AllowAll)
        .await
...
async fn spending_recorded_by_hand_draws_the_same_pool_down() {
    ...
    pool.record(basis_core::RunUsage {
        input_tokens: 1_000,
        ..basis_core::RunUsage::default()
    });
