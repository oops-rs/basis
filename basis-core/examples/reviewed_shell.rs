...
//! [`Approver`](basis_core::Approver). The obvious next question is who answers
...
//! [`OutputSpec`](basis_core::OutputSpec) — no `with_tools()` — and a typed turn
...
//! cargo run -p basis-core --example reviewed_shell -- /tmp/scratch
//! ```
//!
...
// Reached for deliberately, and the one place this example leaves basis's own
// surface: a remembered rule is mentra's type, written through the session basis
...
///
/// Unset by default everywhere in basis, because no unattended run should rely on
...
/// answering ahead of the approver — and basis has no vocabulary of its own for
...
/// by `basis-core`. What makes the rung reachable at all is that
