...
impl PreparedRun {
    ...
    /// }
    ///
    /// # async fn example(run: &mut basis_core::PreparedRun) -> Result<(), basis_core::RunError> {
    /// let spec = basis_core::OutputSpec::new(
    ///     "submit_review",
    ...
    /// run.execute(basis_core::NullSink).await?;
    /// let output = run
    ///     .output::<Review, _, _>(
...
mod tests {
    ...
    fn a_run_that_answered_still_names_the_budget_that_ended_it() {
        ...
        // it, and basis-core exposes neither.
        assert_eq!(
