> **Archived, closed.** This is the ledger of the ADR-0010…0015 redesign: its
> narrative describes the pre-0.7 tree, and its rows are per-commit history —
> not a live plan. The current architecture lives in
> [`../ARCHITECTURE.md`](../ARCHITECTURE.md).

# basis — Redesign plan

> rev 17 · 2026-08-29 · The transition from the P0–P4 harness to the SDK-first
...
> wave.
> **Rev 16 wires ACP cancellation through `/compact`.** `basis-acp` now arms
> the session token for its built-in command and passes it to
> `PreparedRun::compact_with_options`; a `session/cancel` received while the
> summarizer is in flight returns `StopReason::Cancelled`, leaves the
> transcript untouched, and is disarmed before another turn can observe it.
> The duplex regression and routing fix are `c6e80c6` and `45fd5d8`.
> This is no new runtime or public API: basis routes mentra's released,
> boundable compaction at the ACP edge, as ADR-0005 requires.
> **Rev 17 raises the mentra floor to 0.25.0 and closes the stdio caveat.**
> The primary provider stream now observes the run's cancellation token and
> deadline while opening and while waiting for its first model event; basis
> adopts the existing bound errors rather than adding a second timeout surface.
> `McpStdioClient::connect` clears ambient environment, restores the documented
> runnable baseline, layers explicitly named config env, and groups and tears
> down the process tree on disconnect or drop. The baseline removes ambient
> credentials, but this remains host-owned hygiene, not confinement: the server
> retains the host account's filesystem, network, and account authority.
> Streamable HTTP and legacy SSE are unchanged. The adoption is recorded by
> Basis `b9d473e` against released Mentra `2ce0e36`.
...
ADR-0024's adoption wave is the Basis 0.8 runtime contract over Mentra 0.23.3.
It adds lossless in-process observation and a deliberately narrow private-runtime
consume/rebuild lifecycle; the two rows below state the shipped surface and the
boundary it proves.

The audit-fix wave after 0.8.1 is one lane per finding of a whole-tree review,
run in parallel worktrees. Its row states what the lanes are *for*; each lane's
own result lands with its work, and nothing is claimed here on another lane's
behalf.
...
| Cancellation on the public API | 0010 | **Built** (`07cf4d1`) — `TurnOptions::cancellable()` / `stoppable()` / `with_cancel` / `with_stop`, `execute_with_options` and its neighbours on every entry point (since folded into `execute_with_approver_and_options`, the contract unchanged), and `CancellationToken` re-exported under the rule the commit writes down: every mentra type basis's surface makes a caller *name*, basis re-exports [8] |
| Host-defined run contract and lossless observation | 0024 | **Built in 0.8 over Mentra 0.23.3.** `without_discovery`, `with_resolved_model`, exact `ToolRoster::only`, `RunProfile`, complete `TurnOptions`, typed `RunReport::failure`, and `ToolResultPolicy` let a host state the complete contract without ambient widening. `PreparedRun::register_agent_event_tap` forwards complete Mentra `AgentEvent` values unchanged, synchronously and in occurrence order, while the existing `Event`/JSONL schema remains summary-only. Its Basis-owned `AgentEventTapGuard` is opaque and unregisters on drop. |
| Private runtime consume/rebuild | 0024 | **Built in 0.8, now retired** ([ADR-0026](../adr/0026-the-rebuild-half-of-reuse-is-deferred.md)). The recipe / consuming-bind / consuming-rebuild path shipped whole and never acquired a caller: every call site in the workspace was a test or a doc, and the external host ADR-0024 was written for exercises Gate 1a (`fresh_only`) only. It cost ~900 implementation lines, ~1,300 test lines, and 17 of `RunError`'s 45 variants — that size being downstream of one upstream gap, that mentra-provider cannot mint a fresh provider session scope from an existing provider, which forced a type-erased provider factory, a separately deferred async warm step, and a hand-written builder replay. Removed: `RuntimeRecipe`, `with_reusable_registered_provider`, `into_reusable_recipe`, `with_runtime_recipe`, `bind_host_tools`, `rebuild_for_reuse`, and those 17 variants. Gate 1a stays byte for byte; a pooling host opens a fresh workspace per checkout. **Reinstatement trigger**: [oops-rs/mentra#46](https://github.com/oops-rs/mentra/issues/46) closing reopens the decision — the contract would be rebuilt against a fresh-scope primitive, not resurrected. |
| Audit-fix wave (thirteen lanes) | — | **In flight.** One lane per audit finding, each in its own worktree: system-prompt parity, the MCP name rule, `basis-tasks`'s mentra dependency (ADR-0022 decision 4, made true again), read-only lock probes, canonical roots, wait deadlines at turn boundaries, typed failure reports, compaction events, ACP cancel/close against a pending approval, the bridge's open-id set, a no-default-features doc job, `cargo-deny`, and documentation drift. **The docs-drift lane landed here**: ARCHITECTURE.md §4 and containerization.md §1 now state once what a repository's configuration is trusted with — `.mcp.json` connected at `Workspace::open`, hooks asked on every tool call including reads, stdio MCP now clearing ambient environment, restoring the documented baseline, layering explicitly named config variables, and terminating its process tree, all with the operator's authority (ADR-0013) — with the one deliberate exception, a workspace `base_url` refused by name, and the honest admission that `${VAR}` expansion hands the named variables to a repository-declared process; ADR-0016 §7 carries a dated amendment correcting "in front of the same approval" to mentra's real order (authorize, *then* pre-execution hooks, the rewritten input executed unre-authorized) and loses its stale *decided, not built*; the 2026-08-13 concurrency spec is marked superseded by ADR-0019; eight doc comments naming a Mentra 0.22 floor name 0.23; and §7's repository-guard risk stops calling `basis/tests/hooks/` untested. Every other lane's row is its own to write. |
...
| Subprocess bindings on mentra's `BoundedCommand` | 0001/0005/0012 | **Built** (mentra 0.25; hook and declared-tool adoption landed in 0.24, stdio MCP in `2ce0e36`, Basis floor in `b9d473e`) — `basis/src/subprocess.rs` stopped being a `std::process` runner of its own and became basis's use of `mentra::process::BoundedCommand`, the primitive mentra's shell executor runs on. What closed with it: the child's environment is *what basis passes* rather than the parent's (`env_clear`); the deadline kills the whole process group, so `sleep 60 &` in a hook no longer outlives it (the "does not kill descendants" caveat the old module carried is gone); output is capped while it is read (`OUTPUT_CAPTURE_LIMIT`); a dropped run kills the child. The hook path is async end to end — no `spawn_blocking` — and `HookRunner::decide` drives its own runtime on its own thread. basis's part is the **baseline**: `PATH`, `HOME`, `TMPDIR`/`TMP`/`TEMP`, `LANG`/`LC_ALL` (Windows: the set mentra's own tests keep), each named with a reason; a hook gets that and nothing else, a declared tool gets it plus the runtime's command environment and its manifest's `env`. A stdio `.mcp.json` server now uses the same discipline: Mentra clears ambient environment, restores the documented baseline, layers the variables its config explicitly names, and terminates the process tree on disconnect or drop; protocol frames and retained stderr stay bounded and stderr is drained. This removes ambient credentials but is no confinement claim — the server still has the host account's filesystem, network, and account authority. Streamable HTTP and legacy SSE are unchanged. |
...
| A host-constructed provider instance | D5a | **Built** (wave 2) — `RuntimeBuilder::with_provider_instance` surfaces the seam mentra already had: an `impl Provider` is registered under the id its own descriptor reports, models resolve against it, and every run reports that id; the closed enum, the base URL and the environment remain the default path, untouched. Precedence is stated, never ranked: with an instance supplied resolution simply does not run, an explicit `with_provider`/`with_base_url`/`with_api_key` beside one is refused by `build` with `ProviderError::AmbiguousProviderSource` naming the knob — the unattributed-credential posture — and a `config.json`'s provider/base-url yield silently, because a file only ever fills emptiness. What it costs: provider authoring is now basis API — the `Provider` trait at the crate root, the fifteen names an impl's signatures and body touch re-exported in `crate::runtime` beside the executor set, pinned by an end-to-end run in `tests/provider_instance.rs` written against `basis` alone — and `Runtime` carries a `ProviderId` where it carried a `BuiltinProvider`. What it newly exposes: the provider identity is no longer a claim that every provider is builtin — run headers can now carry an id no enum variant spells, and anything downstream that assumed the six names has a counterexample a host can construct. **`#[doc(hidden)]` in the next breaking release** — no production caller, in this workspace or in nous, had constructed a `Provider` yet; only this crate's own tests and `basis-acp/tests/acp/source.rs` call it, across a crate boundary `pub(crate)` cannot reach. Nothing above moves: same signature, same behavior, still `pub` |
| Prompt-before-turn hook | D5b | **Built** (wave 2) — mentra's `RoundStrategy` rides `TurnOptions::with_round_strategy` into `RunOptions` unchanged: a per-call policy consulted after each committed tool round and after the committed final answer, which is where "one more prompt before the next turn" actually lives — `RoundDecision::inject` appends a corrective user turn and runs another round, so a host can refuse an answer and say why before the run returns it. `None` stays the default, and upstream pins that a proceed-everywhere strategy is byte-identical to no strategy, so nothing moves for a caller who asked for nothing. Re-exported at the root: the trait, its context/decision/adjustment vocabulary, and the three types those signatures make an implementor name (`ContentBlock`, `ModelInfo`, `ReasoningOptions`) — `CancellationToken`'s rule, applied to a seam. What it costs: `TurnOptions` writes `Debug` by hand now (presence is all a `dyn` policy can honestly print), and a strategy is per-call like the tokens, deliberately — a configured fallback would share one strategy's state across every turn of a conversation, the exact leak mentra's one-run binding exists to prevent, so a host that wants the same policy each turn attaches it each turn. What it newly exposes: an injected round is a normal round — every bound still binds it, checked before its request goes out (`tests/round_strategy.rs`) |
| Steering mid-turn | D5b | **Built** (wave 2) — the same seam's other half: `RoundAdjustment` switches model or reasoning for the rounds that follow (`ReasoningChange::Set` takes basis's own `Effort` through `.into()`, so raising effort mid-run names no mentra enum), and `RoundDecision::stop` ends the turn gracefully at the boundary. The interplay with the bounds is pinned rather than implied, both ways: a strategy's stop reports `stopped_by: None` — an instruction, not an allowance, exactly as `TurnOptions::stop`, footnote 8's tool-round caveat inherited unchanged — while a budget a proceeding strategy leaves standing still reports `Bound::TokenBudget` as its own, because the runner is what decided. No `Bound` variant was invented for a stop, on the rule footnote 11 already stated: a caller's own decision does not belong on the same exit code as running out of allowance. What it costs: the surface is the SDK's only — no CLI flag reaches a strategy, so footnote 11's silent-exit-`3` edge stays CLI-unreachable and is now exactly one flag away rather than one feature away. What it newly exposes: a host policy can do to a run mid-flight what only mentra's internals could — end it, redirect it, re-model it — and nothing it decides escapes the bounds the turn arrived with |
| `ToolRoster` on `WorkspaceBuilder` | D3 | **Built** (wave 2) — the fixed `REPLACED_TOOLS`/`UNSURFACED_TOOLS` pair moves off `workspace::builder` and into `workspace::roster`, wrapped in `ToolRoster` rather than re-exporting mentra's `ToolProfile` directly: `ToolRoster::default()` reproduces exactly what `agent_config` has always hidden (the existing roster-pinning tests pass unchanged), `hide(names)` extends that set, and `only(names)` maps straight to `ToolProfile::only` — an allow-list basis states no opinion on top of. Three interactions are load-bearing and each is pinned by a test in `workspace::roster::tests`: `only` genuinely stops *offering* a file tool it does not name, but mentra 0.20 has no `FileToolProfile::None`, so the tool stays registered on the runtime underneath — hidden-from-the-roster and gone-from-the-runtime are different claims; `only` implies neither `spawn` nor `load_skill`, so a knowledge agent that wants neither is a real, nameable shape, at the cost of the model reading no such route at all; and the per-mint foreign-tool hide (`Workspace::minted_agent`, a sibling workspace's `mcp__*`/declared names) composes on top of either constructor, because `ToolProfile::allows` checks `hidden_tools` after `allowed_tools` regardless of which one is populated. The rendered prompt — `AGENTS.md`/`CLAUDE.md`, and the memory index — has no view of the roster at all, and stays byte-identical whatever `ToolRoster` a workspace carries, pinned in `workspace::builder::tests`. Re-exported at the root. What it costs: nothing for a caller who never calls `with_tool_roster` — the default is the type's whole reason to exist. What it newly exposes: an allow-list an operator can genuinely narrow a model to, with the file-tool caveat now written down rather than discovered |
| Constants become fields | D9 | **Built** (wave 2), in three places. `SkillsConfig` grows from two fields to four, one per root: `workspace_subdir` and `global_dir` stay `Option<PathBuf>` (`None` disables, `Some` says where), and the two shared `.agents/skills` spellings — the workspace one and `$HOME`'s, previously one unconditional `push` and one gated on `global_dir` alone — become their own `bool` switches, `shared_workspace_dir` and `shared_home_dir`; a caller can now keep the personal `~/.agents/skills` root while disabling basis's own global one, or the reverse, which the coupled `Option` could not express. Default reproduces today's four roots exactly (`skills::tests` pins it), so this is the coordinated three-crate edit the row above declined until something asked for it — `basis-acp` and `basis-cli`'s exhaustive literals, plus every test constructing one, updated in the same commit. `spawn`'s `MAX_DEPTH` constant becomes `RuntimeBuilder::with_delegation_depth`, defaulting to `DEFAULT_DELEGATION_DEPTH` (still 2), threaded into `SpawnTool::with_targets_and_depth` at registration; the guard's shape is untouched — still basis's own ledger, still refusing in the preview, so a remembered allow-rule cannot lift whatever floor is set. And `ContextConfig::none()` turns context discovery off entirely — no `AGENTS.md`, no `CLAUDE.md`, in the workspace, an ancestor, or the global directory — while workspace path validation still runs, reusing the existing `file_name` field (empty means *no name to look for*) rather than adding a fourth field to a struct two sibling crates already construct as an exhaustive literal. What it costs: nothing for a caller who names no override — every default recomputes today's exact discovery set and today's exact depth. What it newly exposes: a host can now build a workspace that reads none of a repository's or a user's skills, delegates only one level deep, or sees no file-derived instructions at all, each independently of the others |
| Host tools claim their name | D5d | **Built** (wave 2) — `RuntimeBuilder::with_tool` used to fold straight onto mentra's builder chain, which registers by `HashMap::insert` and therefore *replaces* on a name collision; a host tool named `spawn` (or any name the runtime already answers to) would silently take over the name and inherit every rule an operator ever wrote about commands and delegation. Host tools are now claimed one at a time, in `build_with`, after the mentra `Runtime` exists and already carries every registration basis makes unconditionally — `spawn`, the builtins, whatever a host-supplied provider's `install` added — using mentra 0.20's `Runtime::try_register_tool` rather than its plain `with_tool`; a collision is `RunError::HostTool` naming the tool, not a swap. The claim is against the live registry and one tool at a time, so a second host tool sharing an earlier one's name collides too, the same posture `Runtime::claim_declared_tool` already holds for a declared tool. What it costs: host tools that used to register during the builder chain now register in a short loop just after `build()`'s own `.build()?` — no behavior change for a caller whose tools never collided, which is every existing test but the two new ones. What it newly exposes: a host learns at `build()` that it shadowed something, instead of learning it the day `spawn` stops working |
| `prepare_with_session` becomes `#[doc(hidden)]` | — | **Built** (wave 2) — the escape hatch existed because `Workspace` could not yet be handed a runtime, a provider instance, a tool roster, or a round strategy, so a bring-your-own-runtime caller had no other way in. That gap is closed: `WorkspaceBuilder::with_runtime`/`with_runtime_builder` (ADR-0018), `with_provider_instance` (D5a), `with_tool_roster` (D3) and `TurnOptions::with_round_strategy` (D5b) cover what the hand-rolled discovery in this function used to be the only way to reach. `#[doc(hidden)]` rather than a removal or a visibility change, because the signature was reshaped last wave to take exactly the inputs its remaining callers use, and those callers are basis's own test harness — `basis/tests/*`, `basis-acp/tests/*`, `basis-cli`'s bridge suite — none of which is a host with a real runtime of its own. A doc comment above the hidden attribute names the real surfaces and says why this one stays. What it costs: nothing compiles differently — `#[doc(hidden)]` is a rustdoc annotation, not a visibility change, so every existing caller is untouched. What it newly exposes: nothing; `cargo doc` simply stops advertising a surface that was never the sanctioned one |
| The memory directory convention (D2's constructive half) | D2 | **Built** (wave 2) — memory decided: files, no database, no tool. One `.md` per memory — frontmatter `name`/`description`/`type` (`user`/`feedback`/`project`/`reference`), body free — under two roots: `memory/` in the global config directory, and the sibling `memory/` beside the store dir when `with_store_dir` named one, so the CLI's layout lands on `<data>/workspaces/<key>/memory` with basis never told the CLI's data dir; ephemeral or default history names no directory basis chose, so the per-workspace root is honestly absent. `Workspace::open` reads frontmatter only and appends an index — name, one line, path — plus a shipped-as-data instruction paragraph, through the context's own render path, so `Replace` removes it, a host's `Append` still lands last, workspace shadows global by name, and zero memories render no block at all. Recall is `read`, search is `grep`, writing one is `write`/`edit`: no new tool, and `run_started` gains nothing — the block is prompt, not schema. On a private runtime both roots join the file tools' allowed read and write roots (in `workspace_policy`, where the `.git` carve-out already extends policy per workspace), stated whether or not the directory exists yet, since the first memory is written by exactly the run that finds none to read; `MemoryConfig` on `WorkspaceBuilder` overrides either root or disables discovery — nothing unconditional (D9). The templates frontmatter scanner moved to `src/frontmatter.rs` and both conventions read it, so the copies stop at two rather than three; mentra's public frontmatter fn stays a pending upstream ask. `examples/memory.rs` files `CompactionCompleted` facts through a sink, `memory::file_contents` being the whole helper. **What it costs**: the index's tokens, in proportion to memories kept, plus one `read_dir` and a frontmatter read per file at open; and a memory that does not parse fails the open naming the file (`.mcp.json`'s rule). **What it newly exposes**: two directories outside the workspace the model can now read and write on a private runtime — stated in policy rather than reached around it, hygiene rather than a boundary (ADR-0013). Shared-runtime posture, corrected post-review (whole-wave review, G2): `BesideStore` derives from the *runtime's* store dir, and on a shared runtime that dir is not any one workspace's — resolving it there gave every workspace on the runtime the identical sibling directory, so unrelated repositories indexed each other's memories into their own prompts. `WorkspaceBuilder::open` now passes no store dir on the shared path at all, so the derived root is honestly absent there, exactly parallel to the dispatcher's existing shared-runtime posture: it can deny a write, never grant a root policy never named. An explicit `WorkspaceMemoryRoot::Dir` is unaffected — naming a path is the host's own responsibility — and `Runtime::store_dir`, left with no caller, is deleted rather than kept unused |
| `PreparedRun::answered_turns` | D8 | **Built** (wave 4) — the one fact `basis-cli`'s resume recovery needed and `basis` did not expose: how many assistant turns a resumed session's history has committed, read against a watermark taken at mint to tell a continued conversation's inherited answers from the crashed task's own. Reading it off `history()` directly meant matching `mentra::Role::Assistant`, which was pulling a durable-task-layer file into a direct `mentra` dependency for one filter-and-count. One method, `history()`'s neighbour on `PreparedRun`, narrower than growing the history element type a role of basis's own — the seam D8's extraction needed first |
| The task layer is a crate (`basis-tasks`) | D8 | **Built** (wave 4) — `data_dir`, `state`, `inbox`, `lock`, `policy`, `events`, `attach`, and the scan half of `tasks` (`TaskSummary`, the workspace scan, continuation resolution) move out of `basis-cli/src/local` into a new crate; `basis list`'s own row/age text formatting and `basis-cli`'s hint/exit-code mapping stay where ADR-0015 puts them. Every cap and rule carries over unchanged: 16 messages, 4 KiB bounded summaries, the finite default deadline, downward-only cancellation, the wait-edge policy. `Tasks` is the new front door — opened per workspace, or at an explicit data-dir root for a host (or a test) that wants no dependency on `BASIS_DATA_DIR` — with `spawn` (mints and returns immediately), `send`/`ask`, `wait`/`wait_message`, `cancel`, `watch` (a pull-based `EventCursor`, never a resident loop), `list`, and the read-only accessors `basis-cli`'s verbs now compose their own rendering from. Two seams a library needs that CLI-only code never did: `PromptHost` for `Approve::Prompt` — a library has no terminal to ask at (ADR-0011's reasoning, one crate over) — and `LiveSink`/`DriveContext`, generalizing the CLI's `Live` progress renderer into something a caller supplies per call rather than something the executor hardcodes. `TaskHandle` replaces a bare `String` at the boundary, validated once; `RunSpec` is the durable spawn request, built the way `basis::RunSpec` is. **What it costs**: one crate more in the workspace graph — four now, still one version (ADR-0011) — and one fact promoted out of the crate's otherwise-flat `Error` (`is_invalid_reference`) so `basis-cli`'s usage-vs-failed exit code distinction survives the boundary without the library knowing what an exit code is. **What it newly exposes**: the durable task layer is reachable from Rust for the first time. `basis-tasks/tests/lifecycle.rs` spawns a task against a scripted endpoint, asks it something, waits for the durable terminal, and lists it back — no `basis` binary anywhere in the process tree, which is the whole point of the extraction, stated as a test |
| basis links no database (the file store, D1's basis half) | D1/0023 | **Built** (wave 4b) — mentra 0.21's `FileRuntimeStore` (upstream `mentra#28`) replaces SQLite through exactly the interface ADR-0023 promised would not move: `with_store_dir` still takes a directory, and that directory is now the store's *root* — `agents/<id>/{agent.json,state.json,transcript.jsonl,leaf}`, `rules.json`, `runs.jsonl` inside it, mentra's own layout, compaction snapshots still a sibling `transcripts/`. The workspace dependency moves to `mentra = 0.21, default-features = false`, which now drops `store-sqlite` beside the websocket transport: `cargo tree` across all four crates shows no rusqlite and no SQLite of any spelling, which is the row's whole claim measured rather than asserted. The `basis:<workspace>` identifier scheme is untouched — the file store keeps the tag as a field of each `agent.json` and filters `list_agents_by_runtime` by comparing it, so the no-hash readable-value ruling holds with less machinery than the hex-encoding it was written against. **No migration** (ADR-0023's E2 precedent), and the refusal is basis's own for a reason found while building: mentra's file store *does* detect a foreign `runtime.sqlite` at `prepare_recovery`, but `RuntimeHandle::prepare_recovery` treats recovery as best-effort and discards the error, so the diagnosis never reaches anyone through `build` — and its wording (enable the `store-sqlite` cargo feature) addresses a mentra embedder, not the person whose conversations are in the file (new upstream candidate: a store diagnosis that only a swallowing path can raise is a diagnosis nobody reads). `store::refuse_legacy_store` therefore runs first in `RuntimeBuilder::build` and under `list`/`forget`: a directory still holding `runtime.sqlite` errors with `RunError::LegacyStore` — the conversations belong to basis 0.6 or earlier, nothing is migrated, continue old work on 0.6, start fresh via `with_store_dir`/`BASIS_DATA_DIR` — instead of an empty file store appearing beside a database the operator would read as their history vanishing. Pinned at three surfaces: the store unit tests, `Workspace::open`, and the binary's stderr (`basis-cli/tests/local_lifecycle.rs`). **What it costs**: the file store keeps tasks, teams and background queues in process memory and *refuses* long-term-memory writes — the second of which basis already decided not to surface (D2), so the one visible residue, a Warning after every compaction's summary ingest naming a mentra cargo feature, is dropped by decision at the event mapping, with the reasoning and a pinning test beside the arm; and the mentra 0.21 re-document forced sixteen latent intra-doc-link errors into the open, repaired in their own commit. **Corrected in review**: the legacy-database refusal was a separate call each opener had to remember, which is a guard a future opener forgets — `store::store_in` is now fallible and does the check itself, so reaching a `FileRuntimeStore` for a named directory without passing it is unspellable. The event mapping's one dropped notice was an unanchored substring match at any severity; it is now `Warning` plus the whole of what upstream composes around the store error, with both halves pinned. **What it newly exposes**: a workspace's entire durable state — tasks, memory, conversations — is now files ordinary tools read, which closes the inconsistency ADR-0023 existed to name |
| The child is a seam (`with_child_policy`) | D4 | **Built** (wave 3) — `RuntimeBuilder::with_child_policy(Fn(&ChildContext) -> ChildSpec)`: per delegation, `spawn` consults the host's policy with what it knows — the child's prompt, the parent's agent id, the workspace directory — and the answer overrides any of the child's three inherited facts: **roster** (the same `ToolProfile` mapping `ToolRoster` has always resolved through, so a policy narrows a child with the vocabulary a host narrows a workspace with), **model** (a `ModelInfo`, not a `ModelSelector` — the honest map onto mentra 0.21's `DisposableSubagentTemplate::with_model`, which resolves the named provider at spawn and fills a missing context window from that provider's listing; a selector would have needed a second resolution pass of basis's own), and **system prompt** (replace-wholesale onto `with_system`; deliberately no `Append` variant, because the parent's rendered prompt is sealed inside mentra's template with no reader, so "parent's plus more" is a promise basis cannot keep — a policy that wants it writes the whole text, and mentra's standard subagent suffix still lands after either way). `ChildSpec::inherit()` — and an unset policy — is 0.6's behavior byte for byte, on the same `spawn_subagent` line rather than the template path with zero overrides, so "identical default" is a fact about the code and not a claim about upstream. Bounds deliberately do not travel through a spec: the child already runs on `child_run_options` (deadline, budgets, cancellation, the shared token counter), and the fan-out accounting test is re-run on the template path to pin that a reshaped child's spend still stops its parent. The approver's preview gains an additive `child` key — `{roster, model, system: "replaced"}`, never the system text — **only when something was overridden**, because the key sorts between `body` and `cwd` and an always-present key would move the serialized shape under every remembered rule already written; a policy'd runtime's inherit answers, and every policy-free runtime, present the exact `{mode, body, cwd, target}` of ADR-0016/0021. Runtime-scoped like the tool that consults it (ADR-0018), so the policy binds at every depth — a child's own delegations are policied too, which is how a chain is confined — and the depth floor is checked before the policy runs on both paths, so no override lifts it. mentra's template-source binding (`SubagentTemplateMismatch`) is structurally unreachable from `spawn` — template taken from and spawned through one `ToolContext` — and would arrive legibly through the standard spawn-failure wrapping if upstream ever changed that. **What it costs**: the policy is consulted twice per approved delegation — once for the preview, once at execution, there being no channel between the two — so it must be a pure function of its context or the approver is shown a child that will not be spawned; and `SpawnTool`/`RuntimeBuilder` write `Debug` by hand now, presence being all a `dyn` policy can honestly print. **Corrected in review, and the correction is the interesting part**: a roster override reached `with_tool_profile`, which *replaces* the child's cloned `ToolProfile` — and part of what it replaced was never the parent's roster at all, but the per-mint hides `Workspace::minted_agent` installs on a shared runtime so one registry's sibling workspaces stay out of each other's rosters. A narrowed child was therefore offered the very tools its parent is denied, another repository's `mcp__*` among them; a `hide` roster is the sharp case, since an allow-list at least omits a sibling's tool by not naming it. mentra 0.21 exposes no reader for an agent's or a template's effective profile (new upstream candidate), so basis carries the denied set along the channel it already has per workspace — the dispatcher's registry entry, written by the same mint that freezes the config — and extends the overriding profile's `hidden_tools` with it. That covers both roster shapes at once because `ToolProfile::allows` checks the denylist after the allow-list: a `hide` gains the names, an `only` that named one loses it silently, which is the rule `ToolRoster::only` already documents for the identical collision on a workspace's own roster. Two smaller corrections rode along: the preview's `model` is provider-qualified (`{id, provider}`, two fields rather than a `provider/id` string, since a single `*` does not cross `/` in mentra's globber and `cwd` already carries that trap), because a policy may route a child to a different vendor and an id alone left the one fact an operator would refuse on invisible; and `with_model` now says to supply `with_context_window`, since mentra fills a missing one by calling the provider's model listing on *every* delegation the override applies to — a network round trip that for Anthropic and OpenAI cannot even answer. **What it newly exposes**: cheap-triage-beside-full-fixer as ~10 lines of host code (`examples/child_policy.rs`), pinned end to end in `tests/child_policy.rs` on a two-provider scripted harness — the triage child really lands on the cheap provider with the narrowed roster and its own voice, an inheriting policy changes nothing observable, and a child of a child still sees one door |
| A workspace hands its skills roots back | — | **Built** (over mentra 0.24's `unregister_skills_dir(s)` and `SkillInfo::root`) — **no ADR: this closes a leak ADR-0018 created and `Workspace::skills` already documented as "an accepted consequence of sharing".** A shared `Runtime` outlives every `Workspace` opened on it, and skills go on the runtime's single registry; with no way to withdraw a root, a host that opened one repository after another accumulated every repository's skills for the life of the process, so a name a run resolved could belong to a checkout closed an hour ago. `SkillRoots` (`basis/src/skills/registration.rs`) is now held by the `Workspace` and releases on drop, the third instance of the shape `DeclaredTools` and `mcp::connections` already have. **The count is basis's own contribution and the thing upstream cannot supply**: mentra keeps one entry per root however often it is registered, and every workspace on a shared runtime registers the same two *user-scoped* roots (`<global config dir>/skills`, `~/.agents/skills`) — unregistering on the first drop would take the user's own skills away from every repository still open. `Runtime::register_skill_roots`/`release_skill_roots` therefore count holders beside the declared-tool claim map, register under the holder lock so no root is ever counted-but-absent, and key the count the way mentra matches a root (canonical path, falling back to the verbatim one for a directory since deleted), so two spellings of one directory are one hold. **What sharing still shares, deliberately**: while two workspaces are both open, a run in one *can* `load_skill` the other's skill — that is real sharing on a shared registry, it is what `Workspace::skills` reports, and it is unchanged; what is new is that it ends when the sibling does, asserted through an actual `load_skill` call rather than a listing (`basis/tests/skills_reclaim.rs`). **A second, smaller leak went with it**: skills are registered before the tool manifests are claimed, so an open refused any time after that point used to leave a shared runtime holding the skills of a workspace that never opened. The hold is a stack value until the `Workspace` takes it, so every `?` in between now gives them back. mentra's own all-or-nothing `register_skills_dirs` covers only the one call; there was no basis-side compensation for partial registration to remove, and both halves are pinned. **Additive on the surface**: `LoadedSkill` gains `root: PathBuf` from `SkillInfo::root`, which is how a host tells this repository's skill from a sibling's now that the list honestly contains both — the same argument that put `model_invocable` there. **What it costs**: `LoadedSkill` has public fields, so a caller constructing one by literal names one more; and `Workspace::skills`'s doc stops claiming it reports "only what this workspace registered", which it never did on a shared runtime |
...
11. **A crossed token budget was a silent success upstream, and is now a
    ...
    **A run that answers *and* is bounded exits `3` printing nothing on
    ...
    steer sitting behind a committed final message, and the CLI has no
    steering surface. Wave 2 gave the SDK one (`TurnOptions::round_strategy`,
    the two D5b rows), where the same result is a `RunReport` a host reads
    whole; the CLI edge stays unreachable until a flag reaches the strategy,
    and it is named here rather than discovered then.
...
nothing on stderr; still unreachable from the CLI now that the SDK has its
steering surface (wave 2), because no flag reaches it — one flag away, not one
feature (footnote 11). And `ModedApprover`'s masking of the remembered
