...
| Structured agent concurrency | 0017 | **Started** — explicit `spawn`/`serve` grammar and actionable `next:` hints are built; the generic lifecycle supervisor now owns attached tasks and accepts detached roots, with downward cancellation and repeatable waits. Independent detached deadlines, durable cross-process state, in-process run wiring, IPC, and messaging remain not started |
