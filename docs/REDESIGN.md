...
> rev 16 · 2026-08-29 · The transition from the P0–P4 harness to the SDK-first
...
| Subprocess bindings on mentra's `BoundedCommand` | 0001/0005/0012 | **Built** (mentra 0.24, the commit that carries these lines) — `basis/src/subprocess.rs` stopped being a `std::process` runner of its own and became basis's use of `mentra::process::BoundedCommand`, the primitive mentra's shell executor runs on. What closed with it: the child's environment is *what basis passes* rather than the parent's (`env_clear`); the deadline kills the whole process group, so `sleep 60 &` in a hook no longer outlives it (the "does not kill descendants" caveat the old module carried is gone); output is capped while it is read (`OUTPUT_CAPTURE_LIMIT`); a dropped run kills the child. The hook path is async end to end — no `spawn_blocking` — and `HookRunner::decide` drives its own runtime on its own thread. basis's part is the **baseline**: `PATH`, `HOME`, `TMPDIR`/`TMP`/`TEMP`, `LANG`/`LC_ALL` (Windows: the set mentra's own tests keep), each named with a reason; a hook gets that and nothing else, a declared tool gets it plus the runtime's command environment and its manifest's `env`. Documented behaviour change, stated in ARCHITECTURE.md §4 and containerization.md §1 with no confinement claim — a hook can still read `~/.netrc`; what stops is the ambient credential. Still upstream's: `.mcp.json` servers are spawned by mentra's `McpStdioClient` (`mcp/client.rs:90-98`) with the environment inherited, so that line in both docs says so rather than pretending. |
