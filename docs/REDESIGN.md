...
| Structured agent concurrency | 0017 | **Started** — explicit `spawn`/`serve` grammar and actionable `next:` hints are built; the generic lifecycle supervisor now owns attached and detached tasks with downward cancellation and repeatable waits. In-process run wiring, IPC, and messaging remain not started |
