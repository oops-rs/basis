...
| Host-defined run contract and lossless observation | 0024 | **Built in 0.8 over Mentra 0.23.3.** `without_discovery`, `with_resolved_model`, exact `ToolRoster::only`, `RunProfile`, complete `TurnOptions`, typed `RunReport::failure`, and `ToolResultPolicy` let a host state the complete contract without ambient widening. `PreparedRun::register_agent_event_tap` forwards complete Mentra `AgentEvent` values unchanged, synchronously and in occurrence order, while the existing `Event`/JSONL schema remains summary-only. Its Basis-owned `AgentEventTapGuard` is opaque, unregisters on drop, and participates in the reuse lifetime below. |
| Private runtime consume/rebuild | 0024 | **Built in 0.8.** `RuntimeBuilder::with_reusable_registered_provider(provider_id, make, warm)` records the host's provider factory and post-build async warm step; Basis checks identity and call order, while a Responses host must return `fresh_session_scope()` and warm that session-sharing clone. `into_reusable_recipe` requires explicit ephemeral history and refuses one-shot providers and builder-bound host tools. `WorkspaceBuilder::with_runtime_recipe` requires discovery-off, fresh-only, resolved-model, exact-roster posture. Every generation starts unbound; consuming `bind_host_tools` validates supplied names and collisions, while the host owns set completeness and roster correspondence. Bind failure returns no entry. Async consuming `rebuild_for_reuse` drops the uniquely owned old runtime before building an unbound replacement and invoking its warm step. Live runs, observer guards, or event forwarders, raw Mentra access, non-unique ownership, and build/warm failure refuse and consume the entry. Team/background/`spawn` execution and detached custom-tool work are excluded host-contract routes rather than names Basis rejects automatically. |
