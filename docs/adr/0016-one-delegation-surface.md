...
because basis leaves them there — `agent_config` (`basis/src/workspace/builder.rs:640`)
...
and `ExecutableTool` (`mentra/src/tool/model.rs:604`) is not among `basis`'s
...
2. **Command mode is put to the `Approver`, always.** `spawn` declares itself
   ...
   reads-are-never-asked rule (`is_consequential`, `basis/src/approval.rs:184`;
...
5. **The policy ladder is existing machinery, tiered.** A remembered rule
   ...
   `deny_and_remember` (`basis/src/run/prepared/forward.rs:145-161`), and
...
6. **Auto-mode is an `Approver` binding, not a new seam.** An implementation
   ...
   (`basis/src/run/output.rs:67-69, 98`), so a reviewer on the default spec
...
- **It depends on a piece ADR-0012 decided and nobody built.** `spawn` is the
  ...
  `basis`'s tool-registration surface — the `ExecutableTool` re-export and a
...
- **Workspace hooks scoped to `shell` stop firing, silently.** A
  ...
  `basis/src/hooks.rs:65`) matches on tool name, and the name the model
...
- **The reviewer runs beside a blocked turn.** The approver is called from the
  ...
  (`basis/src/run/prepared/forward.rs:34-40, 108-138`), so an auto-mode
