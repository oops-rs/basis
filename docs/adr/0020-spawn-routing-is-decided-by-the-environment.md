...
`lan spawn` had three possible fates — run here and answer, mint a checkpoint
...
typing `lan "hi"` got a handle to work that had not started; the script passing
...
"identical to `lan run`" — the attended one-shot that printed the answer.
...
`lan --approve prompt "tidy up the imports"` example could not run at all,
...
`LAN_TASK_ID` is that question, already answered and already in the environment.
...
4. **`--approve prompt` is legal exactly where a process is driving the agent
   ...
   `lan wait`. Only `--resumable` work is refused, having nobody to ask.

## Reconciling ADR-0014

ADR-0014 decided that "an attended `lan spawn` still gets no implicit timer"
and that "with no shipped unattended mode, there is no surface for lan to
...
- `lan "…"` no longer returns immediately at a shell. Callers that relied on
...
- The matrix is a table in `lan/src/route.rs` asserted cell by cell. A grammar
