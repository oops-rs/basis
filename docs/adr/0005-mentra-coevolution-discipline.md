...
The same author owns mentra and basis. That removes the usual friction of upstream
contribution — and the usual forcing function that keeps a consumer honest. The
easy path when basis hits a mentra gap is a basis-side workaround; enough of those and
mentra's API story stops being legible to anyone else (the original zentox problem,
reproduced internally).

## Decision

When basis needs something mentra-shaped, it lands **in mentra**, and it is **filed
...
public skills API, tool profiles, assembly-level test harness. basis may carry a
...
- basis PRs stay reviewable: conventions and protocol only.
