...
//!
//! `basis serve --acp` speaks JSON-RPC on stdin. A bare `basis` invocation is
...
//! Prompt-from-stdin stays explicit at `basis spawn -` (ADR-0017).
//!
...
pub enum StdioError {
    ...
    /// who knows what to suggest instead — the binary names `basis spawn -`, an
...
///
/// This is the transport behind the explicit `basis serve --acp` command. Bare
/// `basis` prints usage; selecting a long-lived protocol server is deliberate
/// (ADR-0017).
///
...
enum Opening {
    ...
    NotAClient,
    /// It sent nothing at all — an editor that spawned basis and closed the
...
/// a known method would put basis in the business of policing a protocol the SDK
