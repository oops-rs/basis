...
- **Five crates, split by dependency weight** (ADR-0011), so new code has one right home:
  `basis` is the SDK and carries no protocol, no transport, and no TTY code; `basis-host`
