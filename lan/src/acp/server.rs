...
//!   (`ANTHROPIC_API_KEY` and the rest, see [`provider`](crate::provider)).
...
/// [`PolicyAuthorizer`](crate::approval::PolicyAuthorizer) at
...
impl ConfiguredSource {
    ...
    fn config_for(&self, cwd: PathBuf, mcp: Vec<McpServer>) -> RunConfig {
        ...
        // answer — see [`mode`](super::mode). A read-only session is the
...
impl SessionSource for ConfiguredSource {
    async fn create(&self, cwd: PathBuf, mcp: Vec<McpServer>) -> Result<PreparedRun, RunError> {
        crate::run::prepare_without_prompt(self.config_for(cwd, mcp)).await
    }
    ...
    async fn resume(
        ...
        crate::run::resume(agent_id, self.config_for(cwd, mcp)).await
    }
...
async fn new_session(
    ...
    // nothing to offer.
    let mcp =
        crate::mcp::from_acp(&request.mcp_servers).map_err(|error| setup_failed(error.into()))?;
    ...
    let commands = crate::templates::available_commands(&run.context().templates);
...
async fn open_persisted(
    ...
    let session = match sessions.get(&session_id) {
        ...
        None => {
            let mcp =
                crate::mcp::from_acp(&mcp_servers).map_err(|error| setup_failed(error.into()))?;
...
fn is_missing_credential(error: &crate::RunError) -> bool {
    matches!(
        error,
        RunError::Provider(
