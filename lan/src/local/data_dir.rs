...
//! store; the repository's `.lan/` stays configuration only. Paths are derived
...
impl DataDir {
    /// Resolves the root: `LAN_DATA_DIR`, else an absolute `XDG_DATA_HOME`,
    /// else the platform data home. Created private (0700) on first use.
    pub(crate) fn discover() -> io::Result<Self> {
        let root = if let Some(data) = std::env::var_os("LAN_DATA_DIR") {
            PathBuf::from(data)
...
fn platform_data_home() -> io::Result<PathBuf> {
    ...
    Err(io::Error::new(
        ...
        "no data directory: set LAN_DATA_DIR",
    ))
}
