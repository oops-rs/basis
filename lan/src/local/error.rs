...
pub(crate) fn message_timeout(task: &str, message_id: &str, timeout: Duration) -> ClientError {
    let message = format!(
        "message {message_id} on {task} timed out after {}; retry with `lan wait {task} --message {message_id}` or inspect `lan inbox {task}`",
        human_duration(timeout)
    );
    let payload = serde_json::json!({
        "error": message,
        ...
        "message": message_id,
        "state": "waiting",
        "next": format!("lan wait {task} --message {message_id}"),
    });
    ClientError::timeout(message, payload)
}
