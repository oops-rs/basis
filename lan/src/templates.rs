...
//! frontmatter describing it. lan finds them, layers them, and substitutes
...
//! the reason they carry names at all — see [`available_commands`].
//!
//! # Where they come from
//!
//! `.lan/templates/` in the workspace, then `templates/` in the global config
...
//! directories joined by `:` — `.lan/templates/git/commit.md` is `git:commit`.
...
pub use command::available_commands;

/// Where lan looks inside a workspace, relative to its root.
pub const DEFAULT_WORKSPACE_TEMPLATES_DIR: &str = ".lan/templates";
...
pub struct Template {
    ...
    /// a client shows this string verbatim, so inventing one would put lan's
...
mod tests {
    ...
    fn a_file_where_the_directory_should_be_is_ignored() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let dir = tmp.path().join(DEFAULT_WORKSPACE_TEMPLATES_DIR);
        std::fs::create_dir_all(dir.parent().expect("parent")).expect("create .lan");
