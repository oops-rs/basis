...
//!
//! What lan does *not* do is kill the hook's descendants. A hook that
...
/// The longest lan waits between checks on a running hook. The poll starts far
...
/// `./.lan/hooks/guard.sh` means what the file says it means regardless of
/// where lan was started. A bare name is left alone for `PATH` to answer, which
