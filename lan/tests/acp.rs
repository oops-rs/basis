...
impl MockSource {
    ...
    fn config(&self) -> RunConfig {
        RunConfig::new(&self.workspace, "").with_context(lan::ContextConfig {
...
impl SessionSource for MockSource {
    ...
    async fn list_sessions(&self, _cwd: PathBuf) -> Result<Vec<lan::PersistedSession>, RunError> {
        Ok(self
            .mock
            .runtime()
            ...
            .map(|agent| lan::PersistedSession {
                agent_id: agent.id,
                name: agent.name,
