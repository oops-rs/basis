...
//! the path from a file on disk (or a client's `session/new`) to the list of
//! servers a runtime would be built from. Spawning a real MCP server would
//! test mentra's client, which mentra already tests.

use std::path::{Path, PathBuf};

use lan::{
    ContextScope, McpConfig, McpError, McpServer, RunConfig,
    mcp::{DEFAULT_GLOBAL_MCP_FILE, DEFAULT_WORKSPACE_MCP_FILE, discover, from_acp, servers},
};
...
fn a_broken_file_never_quotes_what_it_read() {
    // `.mcp.json` is in lan's own `.gitignore`, and in most projects', because
...
fn a_client_supplied_server_wins_over_the_workspaces_own() {
    use agent_client_protocol::schema::v1::{McpServer as AcpServer, McpServerStdio};
    ...
    let found = servers(tmp.path(), &config(None).with_supplied(supplied)).expect("layering");
...
fn a_client_transport_lan_cannot_serve_is_refused_not_ignored() {
    use agent_client_protocol::schema::v1::{McpServer as AcpServer, McpServerHttp};
