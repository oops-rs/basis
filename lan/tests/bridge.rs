...
//!
//! Everything here is genuine except the model: lan's server, the
...
//! serve a client lan does not ship, so the thing worth testing is that a
//! client which knows nothing about lan can drive it end to end.
//!
...
use lan::{
    PreparedRun, RunConfig, RunError,
    acp::{ServeConfig, SessionSource},
    bridge::{Bridge, BridgeConfig, websocket_transport},
...
///
/// Port 0 rather than [`lan::bridge::DEFAULT_PORT`] so tests never collide
