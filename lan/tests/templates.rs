//! Prompt templates end to end: a workspace on disk, the commands a client
//! would see, and the prompt a chosen command produces.
//!
//! The unit tests cover each piece in isolation. What is worth checking from
//! outside the crate is that the pieces agree — that the name discovery derives
//! is the name ACP advertises, that precedence between roots survives loading,
//! and that a broken file stops a run instead of quietly shrinking the command
//! list.
...
use lan::templates::{
    ...
    TemplatesConfig, available_commands,
};
use lan::{ContextScope, RunConfig};
