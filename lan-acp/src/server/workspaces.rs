//! What lan's own sessions are built on: one runtime for the server process,
...
//! [`prepare_without_prompt`](lan_core::run::prepare_without_prompt) per
...
//! `.lan/hooks.json` with it. Nothing here evicts, therefore: `session/close`
//! reaches lan after the fact, and a turn still unwinding would be the one
...
/// The default source: sessions lan builds itself, on the process's runtime.
...
impl ConfiguredSource {
    ...
    /// [`RuntimeBuilder::with_api_key`](lan_core::RuntimeBuilder::with_api_key)
    ...
    /// fixed for its life, so lan installs one that surfaces every
    ...
    async fn workspace_for(
        ...
        // `split` is lan-core's own mapping from a one-prompt config to the
...
impl SessionSource for ConfiguredSource {
    ...
    /// lands upstream — see [`Runtime`](lan_core::Runtime)'s `mint`, the one
...
/// the same command posture, and lan-core's dispatcher — which keys on the
