...
//! for this workspace, in this editor, right now. lan's job is to translate,
//! not to filter.
//!
//! # Nothing is dropped
//!
...
//! lan cannot serve returns an error naming the server and the transport,
...
fn stdio_server(stdio: &McpServerStdio) -> Result<McpServer, McpError> {
    ...
    Ok(McpServer::Stdio(McpServerConfig {
        ...
        // inheriting lan's is the same thing the client's own agent would do.
        cwd: None,
    }))
}
